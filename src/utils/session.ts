...
import { sessionTable, User, userTable, type Session } from '../db/schema/user';
import { db } from '@/db'
...
export async function validateSessionToken(token: string) {
    ...
    const row = await db.select().from(sessionTable).innerJoin(userTable, eq(sessionTable.userId, userTable.id)).where(eq(sessionTable.id, sessionId)).limit(1)
...
export async function setSessionAs2FAVerified(sessionId: string): Promise<void> {
    await db.update(sessionTable).set({isVerified: true}).where(eq(sessionTable.id, sessionId))
